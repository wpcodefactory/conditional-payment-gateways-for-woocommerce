<?php
/**
 * Conditional Payment Gateways for WooCommerce - Module - Product Shipping Class
 *
 * @version 2.4.0
 * @since   2.4.0
 *
...
if ( ! class_exists( 'Alg_WC_CPG_Module_User_Product_Shipping_Class' ) ) :

class Alg_WC_CPG_Module_User_Product_Shipping_Class extends Alg_WC_CPG_Module {
	 ...
	 */
	function get_title() {
		return __( 'Product Shipping Class', 'conditional-payment-gateways-for-woocommerce' );
	}
	 ...
	 */
	function get_desc() {
		return __( 'Hides payment gateways by cart product shipping classes.', 'conditional-payment-gateways-for-woocommerce' );
	}
	 ...
	 * @since   2.4.0
	 *
	 * @todo    (dev) add "No shipping class" option?
	 */
			...
			if ( isset( WC()->cart ) ) {
				foreach ( WC()->cart->get_cart() as $item ) {
					if ( ( $product = $item['data'] ) ) {
						$this->current_value[] = $product->get_shipping_class_id();
					}
				}
...
return new Alg_WC_CPG_Module_User_Product_Shipping_Class();
